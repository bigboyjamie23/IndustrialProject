...
if (!isset($submit))
    $submit = '';
if (!isset($browsestatus))
    $browsestatus = 0;
if (!isset($fkPublisherID))
    $fkPublisherID = 0;
if (!isset($badstatus))
    $badstatus = 0;
if (!isset($subAction))
    $subAction = '';
if (isset($findJournal) && $q != '')
    $action = 'findJournal';
if (!isset($enforce))
    $enforce = 0;
if (!isset($br))
...
require_once($libDir."/journalSearch/journalSearchManager.php");
$thisfile = '';
...
function doAction($db, $indexphp, $action, $loginForm, $SESSION_UNAME, $q = '', $submit = '', $browsestatus = 0, $journalID = 0, $fkPublisherID = 0, $badstatus = 0, $subAction = '', $enforce = 0, $br = '', $dj = '', $folljid = 0, $delete = '', $userType = 'user', $arch = '', $reason = 0)
{
    switch ($action) 
    {
        case "AddJournal":
            ...
            break;
        case "EditJournal":
            $webPage = journalDataManager($db, $action, $q, $submit, $browsestatus, $subAction, $journalID, $fkPublisherID, $userType, 0, $br, $dj, $folljid, $delete, $arch, $reason);
            break;
        case "findJournal":
            $output = journalSearch($db, $q);
            $numJournals = sizeof($output);
            if ($numJournals == 0)
                $webPage = getJournalRecord($db, $browsestatus, 0, false, $subAction, $q, '');
            else if ($numJournals == 1)
                $webPage = getJournalRecord($db, $browsestatus, 0, $output, $subAction, $q);
            else
                $webPage = browseJournals($db, $output, $action, 0, $subAction, $q, $fkPublisherID);
            break;
        case "BrowseSchools":
    ...
    if ($subAction == 'queryFound') {
        if (is_array($res)) {
            ...
            if ($numHits > 0) {
                ...
                $thisSchoolBody = '<p>Found no journals</p>';
        } else {
            ...
            if ($res) {
                ...
                $thisSchoolBody = '<p>You need to specify a search query</p>';
        }
        ...
        if ($db->numrows($res) > 0) {
            ...
            if ($badstatus == 8) {
                $foundthis = 'First <b>';
                $thisSchoolBody = getDOAJrows($db, $res, $action, $fkPublisherID, $badstatus, $indexphp);
            } else
            ...
            $thisSchoolBody = '<p>Found No Journal</p>';
    } else if ($fkPublisherID > 0) {
        ...
        if ($db->numrows($res) > 0) {
            ...
            if ($homePage != '')
                $websites = ' &nbsp; [<a href="' . $homePage . '" title="' . $homePage . '" target="_blank"><span style="color:#ffffff">Website</span></a>]';
            if ($OPMLURL != '')
                $websites .= ' [<a href="' . $OPMLURL . '" title="' . $OPMLURL . '" target="_blank"><span style="color:#ffffff">Logo</span></a>]';
        }
        ...
        if ($db->numrows($res) > 0) {
            ...
            $thisSchoolBody = getBrowseRows($db, $res, $action, $fkPublisherID, $subAction, $badstatus, $orderpubsort, $pubsort, $indexphp);
            if ($thisPublisherName != '')
                $thisSchoolName .= $thisPublisherName . ' ' . $websites;
            else
                $thisSchoolName .= ' this publisher';
        } else
            $thisSchoolBody = '<p>Found No Journal for this Publisher</p>';
    }
...
function getJournals($db)
{
    ...
    if ($db->numrows($res) > 0) {
        while ($row = $db->fetcharray($res)) { //1
            ...
            if (isset($arrayPublishers[$fkPublisherID]))
                $arrayJournals[$journalID]['publisher'] = $arrayPublishers[$fkPublisherID];
...
///////////////////////////////////////////////////////////////////
function getDOAJrows($db, $res, $action, $fkPublisherID, $badstatus, $indexphp)
{
...
ajax;
    ...
    while ($row = $db->fetcharray($res)) { //1
        ...
        if ($printISSN != '')
            $thisJournalID = array_search($printISSN, $bigArray[1]);
        else if ($eISSN != '')
            $thisJournalID = array_search($eISSN, $bigArray[2]);
        ...
        if (strlen($journalTitle) > 40)
            $journalTitleLink = substr($journalTitle, 0, 38) . '...';
        ...
        if (trim($journalHtmlURL) != '')
            $visit = ' <a href="' . $journalHtmlURL . '" title="HomePage" target="_blank"><img src="images/webpagelink.gif" alt="HomePage" title="HomePage" border="0" /></a>';
...
function getBrowseRows($db, $res, $action, $fkPublisherID, $subAction, $badstatus, $orderpubsort, $pubsort, $indexphp, $q = '')
{
    ...
    foreach ($output as $thisres) {
        ...
        //while ($row = $db->fetcharray($res)) { //1
        $journalISSNonline = $journalISSNprint = $journalTitleLink = $publisher = $journalHtmlURL = $journalXmlURL = $journalTitle = $journalClassification = $visit = $journalComments = '';
        $journalRights = $journalID = $followers = 0;
        $journalDescription = '';
        $red = '<img src="images/red.gif" alt="URL missing. Needs attention" title="URL missing. Needs attention" border="0" />';
        $fkPublisherID = $journalStatus = 0;
        ...
        if ($journalRights == 1)
            $JournalPrice = '<img src="images/free.gif" alt="Free" title="Free" border="0">';
        else if ($journalRights == 2)
            $JournalPrice = "<span onclick=\"changeaccess(this, '$journalID')\"><img src=\"images/icon_partial.jpg\" alt=\"Partially Free\" title=\"Partially Free\" border=\"0\"></span>";
        else if ($journalRights == 3)
            $JournalPrice = '<b style="color:#006600">OA</b>';
        else if ($journalRights == 5)
            $JournalPrice = "<span onclick=\"changeaccess(this, '$journalID')\"><img src=\"images/icon_hybrid.gif\" alt=\"Hybrid\" title=\"Hybrid\" border=\"0\"></span>";
        $journalTitleLink = $journalTitle = str_replace('International ', 'Int. ', $journalTitle);
        if (strlen($journalTitle) > 40)
            $journalTitleLink = substr($journalTitle, 0, 38) . '...';
        ...
        if ($fkPublisherID > 0) {
            ...
            if (strlen($publisher) > 36)
                $publisher = substr($publisher, 0, 32) . '...';
        ...
        if ($badstatus == 5 || $badstatus == 6 || $badstatus == 7) {
            ...
            if ($span_title != '')
                $reg = "<span title=\"$span_title\">$followers</span>";
            else
                $reg = $followers;
        } else
            $reg = $arrayJournalStatus[$journalStatus];
        if (trim($journalHtmlURL) != '')
            $visit = ' <a href="' . $journalHtmlURL . '" title="HomePage" target="_blank"><img src="images/webpagelink.gif" alt="HomePage" title="HomePage" border="0" /></a>';
        else
            $visit = $red;
        if (trim($journalXmlURL) != '')
            $visit .= ' <a href="' . $journalXmlURL . '" title="RSS feeds" target="_blank"><img src="images/rsslink.png" alt="RSS feeds" title="RSS feeds" border="0" /></a>';
        else
            $visit .= ' ' . $red;
        if (trim($journalDescription) != '')
            $visit .= ' <a href="' . $journalDescription . '" title="Cover" target="_blank"><img src="images/webpagelink.gif" alt="cover" title="cover" border="0" /></a>';
        else
            $visit .= ' ' . $red;
        $todo = 'archive';
    ...
    if ($submit == 'Save') {
        ...
        if ($msg == '') {
            ...
            if ($journalID > 0) {
                ...
                if ($action == 'EditJournal' && $browsestatus != 3)
                    $msg = 'Journal has been updated';
            } else {
                if ($action == 'EditJournal')
                    $msg .= '- A value for journalID is missing<br>';
                else
                    $msg = '<b>Following errors prevented the creation of a new Journal:</b><br>' . $msg . '<b> Please correct the errors and try to save again</b>';
                $msg = '<p style="color:#cc0000;">' . $msg . '</p>';
            }
        } else
            $msg = '<p style="color:#cc0000;"><b>Following errors prevented the execution of your request:</b><br>' . $msg . '<b> Please correct the errors and try to save again</b></p>';
...
function validateEntryJournal()
{
    ...
    if (!isset($journalDescription))
        $journalDescription = '';
    if ($journalLogoURL != '')
        $msgError .= checkStr($journalLogoURL, '- Invalid OnlineFirst RSS feed URL<br>', '', 10, 500);
    $msgError .= checkStr($journalDescription, '- Missing or Invalid Journal Cover URL<br>', '', 10, 500);
    if ($journalISSNonline != '')
        $msgError .= checkStr($journalISSNonline, '- Invalid online ISSN number<br>', '', 7, 18);
    if ($journalISSNprint != '')
        $msgError .= checkStr($journalISSNprint, '- Invalid print ISSN number<br>', '', 7, 18);
    if (trim($journalISSNonline) == '' && trim($journalISSNprint) == '')
        $msgError .= '- ISSN numbers are missing<br>';
    if (sizeof($journalSubjects) == 0)
        $msgError .= '- Subjects are missing<br>';
    return $msgError;
}
/////////////////////////////////////////////////////////////
function AddUpdateJournal($db, $browsestatus, $journalID = 0, $enforce = 0)
{
    ...
    if (sizeof($journalSubjects) > 0)
        foreach ($journalSubjects as $val)
            $journalClassification .= trim($val) . ' ';
    ...
    if (!findme($journalHtmlURL, 'http'))
        $msg .= "- ERROR: <b>$journalHtmlURL</b> is missing http or https<br>";
    if (!findme($journalXmlURL, 'http'))
        $msg .= "- ERROR: <b>$journalXmlURL</b> is missing http or https<br>";
    if ($journalLogoURL != '') if (!findme($journalLogoURL, 'http'))
        $msg .= "- ERROR: <b>$journalLogoURL</b> is missing http or https<br>";
    if ($journalDescription != '') if (!findme($journalDescription, 'http'))
        $msg .= "- ERROR: <b>$journalDescription</b> is missing http or https<br>";
    $jsubj = '';
    if ($oldJournalDescription != $journalDescription)
        $jsubj = 'journalSubjects="",';
    ...
    if ($journalID > 0) {
        ...
        if ($db->numrows($res) == 0) {
            ...
            if ($browsestatus == 4)
                $sql .= ", journalSubjects = '', journalComments = '' ";
        }
...
function insertNewJournal($db, $sql, $journalTitle, $journalISSNprint, $journalISSNonline, $journalHtmlURL, $journalXmlURL, $fkPublisherID, $enforce = 0)
{
    ...
    if ($newJournal || $enforce == 1) {
        ...
        $msg .= "- It seems that <b>$journalTitle</b> is ALREADY in the Directory or the provided metadata is invalid.<br>";
...
{
    if (isset($uglyDate))
        $niceDate = date("j-M-y", strtotime($uglyDate));
    else
        $niceDate = ' Not yet';
    return $niceDate;
}
...
function displayPage($db, $indexphp, $action, $loginForm = '', $SESSION_UNAME = '', $subAction = '', $webPage = '', $browsestatus = '')
{
    ...
    if ($action != 'logout' && ($action == '' || $action == 'Home'))
        $webPage = getHomePage($db, $indexphp);
...
function getJournalRecord($db, $browsestatus, $journalID = 0, $thisres = false, $subAction = '', $q = '', $msg = '', $userType = 'user', $br = '', $dj = '', $folljid = 0, $delete = '', $arch = '', $enforce = 0, $reason = 0)
{
    ...
    if (!isset($publisher))
        $publisher = '';
    if (!isset($journalClassification))
        $journalClassification = '';
    $arraySubjects = array();
    if (!isset($currentSubjectsList))
        $currentSubjectsList = '';
    $showArch = true;
    if ($journalID > 0) {
        if ($arch == 'yes') {
    ...
    if ($journalID > 0) {
        ...
        foreach ($_GET as $key => $val)
            $$key = $val;
        require('lib/postData.php');
        if (!isset($journalISSNonline))
            $journalISSNonline = '';
        if (!isset($journalISSNprint))
            $journalISSNprint = '';
    ...
    if ($userType == 'user' || $userType == 'dev')
        $enforceNewJournalCreation = '';
    else
        $enforceNewJournalCreation = '<br><input type="checkbox" name="enforce" value="1" /> Force creation';
    if ($journalID > 0) {
        $enforceNewJournalCreation = '';
    ...
    if ($currentSubjectsList != '')
        $currentSubjectsList = "<ol>$currentSubjectsList</ol>\n";
    $indexStatus = "<b>OK</b>";
    $qqs = $db->query("SELECT * FROM sdDictionary WHERE rec_id='$journalID'") or die($db->error());
    if ($db->numrows($qqs) == 0)
        $indexStatus = "<span style=\"color:#cc0000\">missing</span>";
    $JournalPrice = "<span onclick=\"changeaccess(this, '$journalID')\"><img src=\"images/nofree.gif\" alt=\"Subscription/PPV\" title=\"Subscription/Pay Per View\" border=\0\"></span>";
    ...
    if (!isset($journalStatus))
        $journalStatus = '3';
    if (!isset($rssFeedsVersion))
        $rssFeedsVersion = '';
    if (!isset($rssFeedsModules))
        $rssFeedsModules = '';
    if (!isset($journalLastUpdated))
        $journalLastUpdated = '';
    if ($journalLastUpdated == '' || $journalLastUpdated == '0000-00-00')
        $journalLastUpdated = 'Never';
    else
        $journalLastUpdated = getNiceDate($journalLastUpdated);
    if (!isset($rssCount))
        $rssCount = '';
    if (!isset($journalComments))
        $journalComments = '';
    ...
    if ($rssFeedsVersion != '')
        $harvestTxt .= "- Feed Format: $rssFeedsVersion<br>\n";
    if ($rssFeedsModules != '')
        $harvestTxt .= "- Modules: $rssFeedsModules<br>\n";
    if ($rssCount != '')
        $harvestTxt .= "- Items in Feed: $rssCount<br>\n";
...
function getTopMenu($action, $subAction, $indexphp, $browsestatus, $sortby = 0, $colsort = '')
{
    $extraPars = '';
    if ($sortby > 0 || $colsort != '') {
        $extraPars = "&sortby=$sortby&colsort=$colsort";
    }

    if (!isset($subSubAction)) {
        $subSubAction = '';
    }
    ...
    else 
    {
        ...
        {
            $topMenu = $homeLink;
            if (in_array($subAction, $arrayValidSubActions) || ($subSubAction == '' && $subAction == '')) {
                $topMenu .= '&nbsp; &gt; &nbsp;' . $arrayValidActions[$action];
            } 
            else 
            {
                if ($subAction == 'frombrowse') 
                {
                    $topMenu .= '&nbsp; &gt; &nbsp;<a href="' . $indexphp . '?action=BrowseSchools&subAction=byStatus&badstatus=' . $browsestatus . '">' . $arrayValidActions['BrowseSchools'] . '</a>';
                } else {
                    $topMenu .= '&nbsp; &gt; &nbsp;' . $actionLink;
                    if ($subSubAction == '')
                        $topMenu .= '&nbsp; &gt; &nbsp;' . $subAction;
                    else if (array_key_exists($subSubAction, $arrayValidSubSubActions))
                        $topMenu .= '&nbsp; &gt; &nbsp;' . $subActionLink . '&nbsp; &gt; &nbsp;' . $subSubAction;
                }
            }
        }
    }
